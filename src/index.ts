...
const karori_sold = '\"polylines\":["c|pi`@pa{zFaZdR_Cc@wL{Bo@Q?@CBCJ?L?B@@AV?@?@?D?B?@A@?FAFABCFABAB?@AFA@?@?HAB?@@BABABIJQJ??C@EBGBKFABA@EP?D@@AD??BDHLBFBB@@BFDJ?@BF?????DAB?@?@FN??@D?DABCDC@A?G@??I@IAG?AAG?A?C?G@A?ABAB?@@@DH@@BDBBD@??D@FBV@T@FDBBBD?B?BABABCD~@^EjDiAlCKzB_@~Aq@vEEp@Zf@NpABnA^xBGVa@R]Xg@nB?XBNDNDJJP^Rx@d@d@H??@@@@@@?@?@?@u@hASZ??g@L}@GoAe@U???u@@f@f@V|BI|@Ph@l@tAPZLv@Hr@FLTrBy@tA`@tCM`Be@^iA?a@p@e@XCvAHd@c@fC??DxA?d@FJj@Vt@LBPd@THx@z@p@A^qAn@CXHl@HLHNb@bBi@TMDYLiDtA}B~@b@GB?DXF^@F@HFb@@H??D\\\\??@HXITAN?PF`AhA?`A`@b@Ch@a@LQD??FHNPNP@@LNNPPPNPDFA@A?HJLPDDHJPPFFFHNPBDRp@ZDFz@BXl@d@dAXl@n@Nn@?@BJ?BCXF?h@XDTj@jCnAjABBJPzD|A|AhDHpM^lE`@t@XT^f@kA`BA|AOv@@RJP~@hA`@f@_@tC~B`Az@B??hABfCD@?Kt@OfAt@rDdCfEZnAb@dBzBfCCjCd@jC`EtA|F_A??????H@fB@p@?l@@@_@@AvAs@bEZ????f@b@JLbBzAFBB@D@p@Xh@R??JD?@@@DJbCfGHJHHRr@??VdAFP\\\\tAxAf@hCzC??vEp@hC^H@j@HvFbA??nDMjAgCh@mAXwA`AyBq@gA???qAA{J??IoB??QsEA}@?aA??yAo@????y@mEu@q@T}DhBcB???AXcAyBo@s@UICk@Qw@eBGcACWq@i@aDeCOMkDeBy@oCyEGEw@?IEoAuAaCTElIeBjD@tDq@dAq@`Ee@|B[jD?pAc@pAUXxA~@jJ@FBZ|@|IjBvQhDy@h[hIpH{DJG??bKp@zBC??t@?@?lAv@|@J`Aa@xAD??pA`@s@r@uIvIdi@lNHBF@tA^j@NnYtHtNvD|A`@Ld@|DjN??n[kE??|`@qFaEuOCKAADEpBsCwEoDGe@qAaEiByFcBoGIi@Ig@Kw@y@sC{AsEk@yBeBsGsBWoKwIaGkAqQgD{NcDb@}@cReDXw@BIkCi@f@iCs@O_GiADEaJcB@E`CqGwFkAFMtCcHR_@dBd@jGgOjJ_UgKsBqCzGuPaDvAmDsGqARe@d@iAd@iA`BoD@Cs@W{BJ_Ar@UQYMa@[i@a@Ka@BQmBy@aA_@y@k@}DoBj@uA\\\\}@eGiAsGkAi@KyP_DkI{Mo@Dg]{j@wE{@wAY_[aLqPyKuMlIGDq@`@}@j@kHrE"]'

const parm_sold =
    '\"polylines\":["digxF}c}j`@?yub@hxW??xub@ixW?"]'
...
const area = karori_sold
const OUTPUT_PATH = "./data/karori_sold.json"

const getPropertyCards = async (area) => {
    ...
    const promises = pageArray.map((page) => {
        return fetch("https://gateway.homes.co.nz/map/cards", {
            ...
            "body": "{"+area+",\"page\":" + page + ",\"limit\":1000,\"display_rentals\":false,\"for_rent\":false,\"for_sale\":false,\"just_sold\":true,\"off_market\":false}",
            "method": "POST"
        }).then(res => res.json());
    })
...
const getPropertyDetails = async (properties) => {
    ...
    progressBar.start(properties.length, 0);
...
const convertData = (properties_raw) => {
    ...
    return properties_raw.map(p => {
        return {
            ...
            land_value: formatPrice(p.property_details.land_value_digit),
            improvement_value: formatPrice(p.property_details.improvement_value_digit),

            display_estimated_rental_lower_value: formatPrice(p.property_details.display_estimated_rental_lower_value),
            display_estimated_rental_upper_value: formatPrice(p.property_details.display_estimated_rental_upper_value),

            display_estimated_lower_value: formatPrice(p.property_details.display_estimated_lower_value),
            display_estimated_upper_value: formatPrice(p.property_details.display_estimated_upper_value),
...
const run = async () => {
    ...
    const propertyCards = await getPropertyCards(area)
    console.log("1. found properties number ",propertyCards.length)

    console.log("2. getting properties details...")
    ...
    console.log("3. formatting properties details...")
    const refinedProperties = convertData(propertyDetails)
    fs.writeFileSync(OUTPUT_PATH, JSON.stringify(refinedProperties));
    console.log("Done!")
}
